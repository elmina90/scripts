...
MONGODB_VERSION=${MONGODB_VERSION:="2.4.9"}
MONGODB_PORT=${MONGODB_PORT:="27019"}
...
mkdir -p "${MONGODB_DIR}"
wget --continue --output-document "${CACHED_DOWNLOAD}" "https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-${MONGODB_VERSION}.tgz"
